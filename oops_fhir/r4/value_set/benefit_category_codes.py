from pathlib import Path

from fhir.resources.valueset import ValueSet as _ValueSet

from oops_fhir.utils import ValueSet


from oops_fhir.r4.code_system.benefit_category_codes import (
    BenefitCategoryCodes as BenefitCategoryCodes_,
)


__all__ = ["BenefitCategoryCodes"]

_resource = _ValueSet.parse_file(Path(__file__).with_suffix(".json"))


class BenefitCategoryCodes(BenefitCategoryCodes_):
    """
    Benefit Category Codes

    This value set includes examples of Benefit Category codes.

    Status: draft - Version: 4.0.1

    http://hl7.org/fhir/ValueSet/ex-benefitcategory
    """

    class Meta:
        resource = _resource

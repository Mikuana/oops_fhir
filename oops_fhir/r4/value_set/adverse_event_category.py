from pathlib import Path

from fhir.resources.valueset import ValueSet as _ValueSet

from oops_fhir.utils import ValueSet


from oops_fhir.r4.code_system.adverse_event_category import (
    AdverseEventCategory as AdverseEventCategory_,
)


__all__ = ["AdverseEventCategory"]

_resource = _ValueSet.parse_file(Path(__file__).with_suffix(".json"))


class AdverseEventCategory(AdverseEventCategory_):
    """
    AdverseEventCategory

    Overall categorization of the event, e.g. product-related or
situational.

    Status: draft - Version: 4.0.1

    http://hl7.org/fhir/ValueSet/adverse-event-category
    """

    class Meta:
        resource = _resource

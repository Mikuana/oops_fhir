from pathlib import Path

from fhir.resources.valueset import ValueSet as _ValueSet

from oops_fhir.utils import ValueSet


from oops_fhir.r4.code_system.allergy_intolerance_verification_status_codes import (
    AllergyIntoleranceVerificationStatusCodes as AllergyIntoleranceVerificationStatusCodes_,
)


__all__ = ["AllergyIntoleranceVerificationStatusCodes"]

_resource = _ValueSet.parse_file(Path(__file__).with_suffix(".json"))


class AllergyIntoleranceVerificationStatusCodes(
    AllergyIntoleranceVerificationStatusCodes_
):
    """
    AllergyIntolerance Verification Status Codes

    Preferred value set for AllergyIntolerance Verification Status.

    Status: draft - Version: 4.0.1

    http://hl7.org/fhir/ValueSet/allergyintolerance-verification
    """

    class Meta:
        resource = _resource
